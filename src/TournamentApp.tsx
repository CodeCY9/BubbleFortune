...
function TournamentSpectatorDuelPanel({ duel, lowQuality, reducedMotion }: { duel: import('../packages/protocol/src/duel').DuelPublicSnapshot; lowQuality: boolean; reducedMotion: boolean }) {
  return <section style={panelStyle}>
    ...
    <div style={gridStyle}>{duel.boxes.map((box) => <div key={box.id} style={{ minHeight: '62px', borderRadius: '10px', border: '1px solid rgba(255,255,255,.12)', background: box.status === 'opened' ? 'rgba(34,197,94,.15)' : box.status === 'selected' ? 'rgba(245,158,11,.2)' : 'rgba(255,255,255,.05)', color: '#fff', display: 'grid', placeItems: 'center' }}>#{box.id}<br />{box.status === 'opened' ? formatMoney(box.revealedAmount) : box.status}</div>)}</div>
...
function TournamentDuelPanel({ duel, role, offerAmount, setOfferAmount, onCommand, lowQuality, reducedMotion }: { duel: DuelClientSnapshot; role: string | null; offerAmount: string; setOfferAmount: (value: string) => void; onCommand: (type: DuelActionType, extra?: Record<string, unknown>) => void; lowQuality: boolean; reducedMotion: boolean }) {
  ...
  return <section style={panelStyle}>
    ...
    <div style={gridStyle}>{duel.public.boxes.map((box) => <button key={box.id} type="button" disabled={animating || (!canSelect && !canOpen)} onClick={() => onCommand(canSelect ? 'SELECT_BOX' : 'OPEN_BOX', { boxId: box.id })} style={{ minHeight: '62px', borderRadius: '10px', border: '1px solid rgba(255,255,255,.12)', background: box.status === 'opened' ? 'rgba(34,197,94,.15)' : box.status === 'selected' ? 'rgba(245,158,11,.2)' : 'rgba(255,255,255,.05)', color: '#fff', cursor: animating ? 'not-allowed' : 'pointer' }}>#{box.id}<br />{box.status === 'opened' ? formatMoney(box.revealedAmount) : box.status}</button>)}</div>
...
function TournamentSpectatorSurvivorPanel({ survivor, lowQuality, reducedMotion }: { survivor: SurvivorPublicSnapshot; lowQuality: boolean; reducedMotion: boolean }) {
  const language = useLanguage();
  return <section style={panelStyle}><div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}><h2 style={{ marginTop: 0 }}>{translateTournament('spectatorSurvivor', language)}</h2><span style={{ color: '#fcd34d' }}>{survivor.phase}</span></div><MultiplayerStage3D boxes={survivor.boxes} personalBoxId={null} phase={survivor.phase} currentChooserSeatId={null} mySeatId={null} allowedActions={[]} onSelectBox={() => undefined} onOpenBox={() => undefined} playerBoxLabel={translateTournament('personalCase', language)} lowQuality={lowQuality} reducedMotion={reducedMotion} /><div style={gridStyle}>{survivor.boxes.map((box) => <div key={box.id} style={{ minHeight: '62px', borderRadius: '10px', border: '1px solid rgba(255,255,255,.12)', background: box.status === 'opened' ? 'rgba(34,197,94,.15)' : box.status === 'selected' ? 'rgba(245,158,11,.2)' : 'rgba(255,255,255,.05)', color: '#fff', display: 'grid', placeItems: 'center' }}>#{box.id}<br />{box.status === 'opened' ? formatMoney(box.revealedAmount) : box.status}</div>)}</div><p style={{ color: '#94a3b8' }}>{translateTournament('hiddenSurvivor', language)}</p></section>;
}
